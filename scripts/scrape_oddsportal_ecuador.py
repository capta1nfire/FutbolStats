#!/usr/bin/env python3
"""
Scrape 1X2 odds from OddsPortal Ecuador Liga Pro (2019-2025).

Multi-season scraper with:
  - Auto page detection (stops when page returns 0 matches)
  - Date extraction from date header divs
  - Per-season output files compatible with ingest_oddsportal.py
  - Checkpoint/resume to handle interruptions

Output: data/oddsportal_raw/ecuador-liga-pro_{year}.json
"""
...
# URL patterns: current year uses "liga-pro", past years "liga-pro-{year}"
def season_url(year: int, page: int) -> str:
    current_year = 2026
    if year == current_year:
        slug = "liga-pro"
    else:
        slug = f"liga-pro-{year}"
    return f"https://www.oddsportal.com/football/ecuador/{slug}/results/#/page/{page}/"


MAX_PAGES_PER_SEASON = 10  # Ecuador ~240 matches/season, ~50/page = ~5 pages
...
CHECKPOINT_PATH = OUTPUT_DIR / "ecuador_checkpoint.json"
...
() => {
  ...
  const getMatchAnchor = (row) => {
    ...
    const footballAnchor = anchors.find((a) => {
      ...
      return href.includes("/football/ecuador/");
    });
  ...
  const extractOdds = (row, structured) => {
    ...
    // 2) Fallback: tokens after away team
    const values = [];
  ...
  // ── Date extraction: walk sibling elements to find date context ──
  ...
  const parseDateHeader = (text) => {
    // Format: "14 Dec 2024" or "01 Jan 2025"
  ...
  // Build a map: for each eventRow, extract date from its internal bg-gray-light child.
  // OddsPortal structure: date headers are INSIDE eventRow divs as children.
  // Some eventRows contain a bg-gray-light div with "14 Dec 2024  - Play Offs".
  // When present, it sets the date for that row and all subsequent rows until next date.
...
async def scrape_season(browser, year: int, ckpt: dict) -> list[dict[str, Any]]:
    """Scrape all pages for one season."""
    ...
    if str(year) in partial:
        ...
        # Load already-scraped matches for this season
        output_path = OUTPUT_DIR / f"ecuador-liga-pro_{year}.json"
...
async def main() -> None:
    ...
    print(f"Ecuador Liga Pro OddsPortal Scraper")
    ...
    async with async_playwright() as p:
        ...
        for year in remaining:
            ...
            print(f"\n  Season {year} DONE: {len(matches)} matches, "
                  ...
                  f"{n_dates} with dates" if matches else f"\n  Season {year} DONE: 0 matches")
    ...
    for year in SEASONS:
        ...
        if output_path.exists():
            ...
            print(f"  {year}: {n} matches, {n_o} with odds ({n_o/n*100:.0f}%)" if n > 0 else f"  {year}: 0 matches")
    print(f"\nOutput: {OUTPUT_DIR}/ecuador-liga-pro_*.json")
