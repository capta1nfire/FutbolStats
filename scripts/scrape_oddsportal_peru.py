#!/usr/bin/env python3
"""
Scrape 1X2 odds from OddsPortal Peru Liga 1 (2019-2026).
...
Output: data/oddsportal_raw/peru-liga-1_{year}.json
"""
...
# URL pattern: liga-1 (current year no suffix, historical with year suffix)
def season_url(year: int, page: int) -> str:
    ...
    if year == current_year:
        ...
        slug = f"liga-1-{year}"
    return f"https://www.oddsportal.com/football/peru/{slug}/results/#/page/{page}/"


MAX_PAGES_PER_SEASON = 10  # Peru ~300-350 matches/season, ~50/page = ~7 pages
...
CHECKPOINT_PATH = OUTPUT_DIR / "peru_checkpoint.json"
...
() => {
  ...
  const getMatchAnchor = (row) => {
    ...
    const footballAnchor = anchors.find((a) => {
      ...
      return href.includes("/football/peru/");
    });
...
async def scrape_season(browser, year: int, ckpt: dict) -> list[dict[str, Any]]:
    ...
    if str(year) in partial:
        ...
        output_path = OUTPUT_DIR / f"peru-liga-1_{year}.json"
...
async def main() -> None:
    ...
    print(f"Peru Liga 1 OddsPortal Scraper")
    ...
    print(f"\nOutput: {OUTPUT_DIR}/peru-liga-1_*.json")
