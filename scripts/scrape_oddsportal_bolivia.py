#!/usr/bin/env python3
"""
Scrape 1X2 odds from OddsPortal Bolivia División Profesional (2019-2025).
...
Output: data/oddsportal_raw/bolivia-division-profesional_{year}.json
"""
...
# URL pattern: division-profesional (current/2025 no suffix, historical with year suffix)
def season_url(year: int, page: int) -> str:
    if year == 2025:
        slug = "division-profesional"
    else:
        slug = f"division-profesional-{year}"
    return f"https://www.oddsportal.com/football/bolivia/{slug}/results/#/page/{page}/"


MAX_PAGES_PER_SEASON = 10  # Bolivia ~180-363 matches/season, ~50/page = ~7 pages max
...
CHECKPOINT_PATH = OUTPUT_DIR / "bolivia_checkpoint.json"
...
() => {
  ...
  const getMatchAnchor = (row) => {
    ...
    const footballAnchor = anchors.find((a) => {
      ...
      return href.includes("/football/bolivia/");
    });
...
async def scrape_season(browser, year: int, ckpt: dict) -> list[dict[str, Any]]:
    ...
    if str(year) in partial:
        ...
        output_path = OUTPUT_DIR / f"bolivia-division-profesional_{year}.json"
...
async def main() -> None:
    ...
    print(f"Bolivia División Profesional OddsPortal Scraper")
    ...
    print(f"\nOutput: {OUTPUT_DIR}/bolivia-division-profesional_*.json")
