#!/usr/bin/env python3
"""
Worker Fixtures/Teams: LATAM Pack2 Backfill
...
Scope Pack2:
- Tier 1: 265 (Chile), 239 (Colombia), 13 (Libertadores)
- Tier 2: 281 (Peru), 242 (Ecuador), 268 (Uruguay Ap), 270 (Uruguay Cl), 11 (Sudamericana)
- Tier 3: 250 (Paraguay Ap), 252 (Paraguay Cl), 344 (Bolivia), 299 (Venezuela)
- Seasons: 2019-2024
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"logs/latam_pack2_backfill_{TIMESTAMP}.log")
    ]
)
logger = logging.getLogger(__name__)

# LATAM Pack2 Configuration - AUTHORIZED SCOPE ONLY
BACKFILL_CONFIG = {
    # Tier 1 - High Priority
    265: {"name": "Chile Primera Division", "country": "Chile", "tier": 1},
    239: {"name": "Colombia Primera A", "country": "Colombia", "tier": 1},
    13: {"name": "CONMEBOL Libertadores", "country": "World", "tier": 1},
    # Tier 2 - Medium Priority
    281: {"name": "Peru Primera Division", "country": "Peru", "tier": 2},
    242: {"name": "Ecuador Liga Pro", "country": "Ecuador", "tier": 2},
    268: {"name": "Uruguay Primera - Apertura", "country": "Uruguay", "tier": 2},
    270: {"name": "Uruguay Primera - Clausura", "country": "Uruguay", "tier": 2},
    11: {"name": "CONMEBOL Sudamericana", "country": "World", "tier": 2},
    # Tier 3 - Lower Priority
    250: {"name": "Paraguay Division Profesional - Apertura", "country": "Paraguay", "tier": 3},
    252: {"name": "Paraguay Division Profesional - Clausura", "country": "Paraguay", "tier": 3},
    344: {"name": "Bolivia Primera Division", "country": "Bolivia", "tier": 3},
    299: {"name": "Venezuela Primera Division", "country": "Venezuela", "tier": 3},
}
...
async def main():
    ...
    try:
        async with async_session() as session:
            ...
            # Process by tier order
            sorted_leagues = sorted(BACKFILL_CONFIG.items(), key=lambda x: x[1]["tier"])
    ...
    report_path = os.path.join(logs_dir, "latam_pack2_coverage_report.json")
    with open(report_path, "w") as f:
        json.dump(combined_report, f, indent=2, default=str)

    # Print summary
    print("\n" + "=" * 70)
    print("LATAM PACK2 BACKFILL SUMMARY")
    print("=" * 70)
    print(f"Duration: {duration:.1f} seconds")
    print(f"Seasons: {SEASONS}")
    print()

    print(f"{'ID':<6} {'Liga':<35} {'New':<8} {'Total':<8} {'FT%':<8} {'Teams':<6} {'Err'}")
    print("-" * 85)

    for stats in all_stats:
        print(f"{stats['league_id']:<6} {stats['league_name']:<35} "
    ...
    print(f"{'TOTAL':<6} {'':<35} {totals['total_matches_new']:<8} {'':<8} "
