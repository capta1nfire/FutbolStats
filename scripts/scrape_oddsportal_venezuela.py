#!/usr/bin/env python3
"""
Scrape 1X2 odds from OddsPortal Venezuela Primera División / Liga FUTVE (2021-2026).
...
Output: data/oddsportal_raw/venezuela-primera-division_{year}.json
"""
...
# URL patterns: name changed from "primera-division" to "liga-futve" in 2023
def season_url(year: int, page: int) -> str:
    current_year = 2026
    if year == current_year:
        slug = "liga-futve"
    elif year >= 2023:
        slug = f"liga-futve-{year}"
    else:
        slug = f"primera-division-{year}"
    return f"https://www.oddsportal.com/football/venezuela/{slug}/results/#/page/{page}/"


MAX_PAGES_PER_SEASON = 10  # Venezuela ~230-300 matches/season, ~50/page = ~6 pages
...
CHECKPOINT_PATH = OUTPUT_DIR / "venezuela_checkpoint.json"
...
() => {
  ...
  const getMatchAnchor = (row) => {
    ...
    const footballAnchor = anchors.find((a) => {
      ...
      return href.includes("/football/venezuela/");
    });
...
async def scrape_season(browser, year: int, ckpt: dict) -> list[dict[str, Any]]:
    ...
    if str(year) in partial:
        ...
        output_path = OUTPUT_DIR / f"venezuela-primera-division_{year}.json"
...
async def main() -> None:
    ...
    print(f"Venezuela Primera División OddsPortal Scraper")
    ...
    print(f"\nOutput: {OUTPUT_DIR}/venezuela-primera-division_*.json")
