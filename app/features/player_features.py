...
async def get_team_absences(
    ...
    Returns:
        ...
        n_questionable: int  — injury_type = 'Questionable'
    """
    if not fixture_external_id:
        return {"n_missing": 0, "n_questionable": 0}

    result = await session.execute(
        text("""
            SELECT
                COUNT(*) FILTER (WHERE injury_type = 'Missing Fixture') AS n_missing,
                COUNT(*) FILTER (WHERE injury_type = 'Questionable') AS n_questionable
    ...
    if row:
        return {
            ...
            "n_questionable": row.n_questionable or 0,
        }
    return {"n_missing": 0, "n_questionable": 0}
...
async def get_player_manager_features(
    ...
    try:
        ...
        has_data = (
            home_absences["n_missing"] > 0
            or away_absences["n_missing"] > 0
            or home_manager["manager_tenure_days"] != 365
            or away_manager["manager_tenure_days"] != 365
        )

        return {
            "home_n_missing": home_absences["n_missing"],
            "away_n_missing": away_absences["n_missing"],
            "home_n_questionable": home_absences["n_questionable"],
            "away_n_questionable": away_absences["n_questionable"],
            ...
            "player_manager_missing": 0 if has_data else 1,
        }
...
def get_player_manager_defaults() -> dict:
    ...
    return {
        "home_n_missing": 0,
        "away_n_missing": 0,
        "home_n_questionable": 0,
        "away_n_questionable": 0,
