...
-- 1. is_active = TRUE means league is SERVED to iOS users (product decision)
...
CREATE TABLE IF NOT EXISTS admin_leagues (
    ...
    -- Activation (CRITICAL: is_active = TRUE means SERVED to iOS users)
...
COMMENT ON TABLE admin_leagues IS 'Source of truth for league configuration. is_active=TRUE means served to iOS users.';
...
COMMENT ON COLUMN admin_leagues.is_active IS 'CRITICAL: TRUE = league served to iOS users (product decision, not temporal)';
...
COMMENT ON COLUMN admin_leagues.tags IS 'Extensible metadata (JSONB)';
