...
/**
 * Proxy route handler for /dashboard/settings/summary.json
 *
...
function generateRequestId(): string {
  try {
    return `settings-summary-${randomUUID()}`;
  } catch {
    return `settings-summary-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
/**
 * Scrub any unexpected PII/sensitive fields from response.
 * Settings summary should be safe, but defense in depth.
 */
function scrubSensitive(response: unknown): unknown {
  ...
  // Remove any unexpected sensitive-looking keys at root or nested levels
...
export async function GET() {
  ...
  const url = `${BACKEND_BASE_URL}/dashboard/settings/summary.json`;
