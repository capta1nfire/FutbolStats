...
/**
 * Proxy route handler for /dashboard/settings/model_versions.json
 *
...
function generateRequestId(): string {
  try {
    return `settings-models-${randomUUID()}`;
  } catch {
    return `settings-models-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
export async function GET() {
  ...
  const url = `${BACKEND_BASE_URL}/dashboard/settings/model_versions.json`;
