...
/**
 * Proxy route handler for /dashboard/settings/ia-features/prompt-template.json
 *
 * GET: Fetch current LLM prompt template
 */
...
function generateRequestId(): string {
  try {
    return `prompt-tpl-${randomUUID()}`;
  } catch {
    return `prompt-tpl-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
/**
 * GET /api/settings/ia-features/prompt-template
 *
 * Fetch current LLM prompt template (read-only).
 */
export async function GET() {
  ...
  const url = `${BACKEND_BASE_URL}/dashboard/settings/ia-features/prompt-template.json`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      return NextResponse.json(data, {
        status: 200,
        headers: {
          "Cache-Control": "public, max-age=300", // Cache for 5 min (template rarely changes)
          "x-request-id": requestId,
        },
      });
    }
