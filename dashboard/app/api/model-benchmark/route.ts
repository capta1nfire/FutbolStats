...
/**
 * Proxy route handler for /dashboard/model-benchmark
 *
...
const TIMEOUT_MS = parseInt(process.env.OPS_TIMEOUT_MS || "15000", 10); // 15s for potentially slow query
...
function generateRequestId(): string {
  try {
    return `mbm-${randomUUID()}`;
  } catch {
    return `mbm-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
