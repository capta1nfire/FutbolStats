...
/**
 * Proxy route handler for /predictions/upcoming
 *
 ...
 * - Passes through query params for filtering
 */
...
export async function GET(request: NextRequest) {
  ...
  const leagueIds = searchParams.get("league_ids");

  if (daysBack) backendParams.set("days_back", daysBack);
  if (daysAhead) backendParams.set("days_ahead", daysAhead);
  if (leagueIds) backendParams.set("league_ids", leagueIds);

  const queryString = backendParams.toString();
  const url = `${BACKEND_BASE_URL}/predictions/upcoming${queryString ? `?${queryString}` : ""}`;

  // Build headers (same pattern as /api/ops)
