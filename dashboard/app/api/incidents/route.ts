...
/**
 * Proxy route handler for /dashboard/incidents.json
 *
...
function generateRequestId(): string {
  try {
    return `incidents-${randomUUID()}`;
  } catch {
    return `incidents-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const url = `${BACKEND_BASE_URL}/dashboard/incidents.json${queryString ? `?${queryString}` : ""}`;
