...
/**
 * Proxy route handler for /dashboard/ml_health.json
 *
...
function generateRequestId(): string {
  try {
    return `mlh-${randomUUID()}`;
  } catch {
    return `mlh-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
export async function GET() {
  ...
  const url = `${BACKEND_BASE_URL}/dashboard/ml_health.json`;
