...
/**
 * Proxy route handler for /dashboard/ops/predictions_performance.json
 *
 ...
 * - Passes through query params (window_days)
 */
...
function generateRequestId(): string {
  try {
    return `analytics-perf-${randomUUID()}`;
  } catch {
    return `analytics-perf-${Date.now()}-${process.pid}`;
  }
}
...
export async function GET(request: NextRequest) {
  ...
  if (windowDays) backendParams.set("window_days", windowDays);

  const queryString = backendParams.toString();
  const url = `${BACKEND_BASE_URL}/dashboard/ops/predictions_performance.json${queryString ? `?${queryString}` : ""}`;
