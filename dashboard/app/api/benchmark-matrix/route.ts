...
/**
 * Proxy route handler for /dashboard/benchmark-matrix
 *
 * Same pattern as model-benchmark proxy:
...
function generateRequestId(): string {
  try {
    return `bmx-${randomUUID()}`;
  } catch {
    return `bmx-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  }
}
...
export async function GET(request: Request) {
  ...
  // Forward period param to backend
  const { searchParams } = new URL(request.url);
  const periodParam = searchParams.get("period");
  const queryString = periodParam ? `?period=${encodeURIComponent(periodParam)}` : "";
  const url = `${BACKEND_BASE_URL}/dashboard/benchmark-matrix${queryString}`;
  ...
  const fetchOptions: RequestInit = { method: "GET", headers, cache: "no-store" };
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (attempt === 0 && isRetryableError(null, response)) continue;
      ...
      if (attempt === 0 && isRetryableError(error)) continue;
